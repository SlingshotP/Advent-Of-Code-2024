        ...
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < data.Length; i++)
            {
                ...
                for (int j = 0; j < combinations.Length; j++)
                {
                    ...
                    {
                        if (operators[k] == '+') acc = acc + operands[k + 1]; // acc = operands[k] + operands[k + 1]     operands[k + 1] + operands[k + 2]
                        else if (operators[k] == '*') acc = acc * operands[k + 1];
                        else
                        {
                            acc = long.Parse($"{acc}{operands[k + 1]}");
                        }
                    }
                    if (acc == result)
                    {
        ...
        static IEnumerable<IEnumerable<char>> CombifyTmp(int remaining)
        {
            ...
            else
            {
                ...
                foreach (var item in rest)
                {
                    ...
                    yield return item.Append('*');
                    yield return item.Append('|'); // Just calling it a single | to keep it as a char
                }
            }
        }
